...
Outputs:
  data/eval_pipeline_a_single_turn.json   (Pipeline A: 50 single-turn queries)
  data/eval_pipeline_b_multi_turn.json      (Pipeline B: 4 multi-turn scenarios)

Run:
  python scripts/generate_eval_dataset.py

Override LLM model:
  EVAL_LLM=qwen3:8b python scripts/generate_eval_dataset.py
"""
...
OUT_PIPELINE_A     = os.path.join(BASE_DIR, "data", "eval_pipeline_a_single_turn.json")
OUT_PIPELINE_B     = os.path.join(BASE_DIR, "data", "eval_pipeline_b_multi_turn.json")
EVAL_A_CHECKPOINT  = os.path.join(BASE_DIR, "data", "eval_a_checkpoint.json")  # resume support
...
def ping_ollama():
    """No longer needed since model is loaded locally."""
    pass
...
"""),
    ...
    # Parse the JSON from LLM output (after stripping thinking blocks)
...
- Jangan beri penjelasan apapun, hanya array JSON."""),
    ...
    for i in range(0, len(pool), chunk_size):
        ...
        for c in chunk:
            f = c['features']
            ctx += f"- Nama: {c['place_name']}\n  Lokasi: {c.get('city_regency', '')}\n  Ringkasan: {f['summary']}\n  Fasilitas: {f['landscape_content_features']}\n  Aktivitas: {f['activity_features']}\n  Suasana: {f['atmosphere_features']}\n\n"
        
        try:
            raw = chain.invoke({"query": query, "context": ctx})
            raw = strip_thinking(raw)
            # Find JSON array
            import json
            import re
...
def build_pipeline_a(checkpoint: dict) -> list:
    ...
    # ── Attribute-Driven Grouping (category + city_regency) ──────────
    # Used for Level 1-2: pick ALL entities in one group as initial ground truth
    by_attribute: dict[tuple, list] = {}
    for e in entities:
        cat  = e.get("category",    "Lainnya")
        city = e.get("city_regency", "")
        by_attribute.setdefault((cat, city), []).append(e)

    # Only use groups with ≥ 2 entities (enough to be meaningful)
    rich_groups = [(k, v) for k, v in by_attribute.items() if len(v) >= 2]
    random.shuffle(rich_groups)
    print(f"   Attribute groups tersedia: {len(rich_groups)} (category × city_regency)")

    # ── Category-only grouping for Level 3-5 cross-category seeds ────
    ...
    for level in range(1, 6):
        ...
        while len(queries_this_level) < 10 and attempts < max_attempts:
            ...
            if level <= 2:
                # ── ATTRIBUTE-DRIVEN: pick one (category, city) group ────────
                # Prefer unused groups for diversity
                unused = [(k, v) for k, v in rich_groups if k not in used_group_keys]
                if not unused:
                    used_group_keys.clear()
                    unused = rich_groups

                group_key, group_entities = random.choice(unused)
                ...
                print(f"      [Group] {cat_name} @ {city_name or 'unspecified'} "
                      f"({len(group_entities)} entitas)")
            else:
                # ── CROSS-CATEGORY: pick seeds from different categories ──────
                ...
                if len(seeds) < 1:
                    continue
                initial_gts = [s["place_name"] for s in seeds]
...
SCENARIO_TEMPLATES = [
    ...
    },
    {
        ...
        "pivot_categories": ["Wisata Budaya & Sejarah"],
    },
    {
        "id": "scenario_b_003",
    ...
    },
    {
        ...
        "pivot_categories": ["Akomodasi", "Restoran (Halal)", "Pusat Oleh-Oleh"],
    },
]
